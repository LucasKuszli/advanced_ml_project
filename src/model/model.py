...
from src.model.input_stem import PiecePlaneStem, SquareTokenStem
...
class ChessTransformer(nn.Module):
    ...
    Args:
        stem (nn.Module): An input stem (``PiecePlaneStem``
            or ``SquareTokenStem``) that maps encoder
...
def build_model(
    ...
    Args:
        encoder_name (str): One of ``"piece_plane"`` or
            ``"square_token"``.
        d_model (int): Embedding dimension (default=256).

...
from src.data.dataset import build_loader, build_precomputed_loader
from src.data.precompute import precomputed_split_dir
...
def train(
    ...
    # Data — prefer precomputed .npy files when available.
    _split_dir = split_dir if split_dir is not None else Path("data/splits")
    pc_dir = precomputed_split_dir(cfg.encoder_name, _split_dir)
    if (pc_dir / "train_X.npy").exists():
        print(f"[train] using precomputed data from {pc_dir}")
        train_loader = build_precomputed_loader(
            "train",
            precomputed_dir=pc_dir,
            batch_size=cfg.batch_size,
            num_workers=cfg.num_workers,
        )
        val_loader = build_precomputed_loader(
            "val",
            precomputed_dir=pc_dir,
            batch_size=cfg.batch_size,
            num_workers=cfg.num_workers,
